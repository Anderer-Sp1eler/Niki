...
type LoadedAnimation = { images: HTMLImageElement[]; frameDuration: number };

const ANIMATIONS: Record<SheepState, { count: number; frameDuration: number }> = {
...
// Diese Werte werden verwendet, damit transparente Randbereiche nicht zu
// einem sichtbaren "Zucken" führen.
const VISUAL_ANCHORS: Record<SheepState, Array<[number, number]>> = {
  idle: [[63.5, 120], [64.5, 120], [64.0, 120], [64.0, 120], [63.5, 120], [63.5, 120]],
  walk: [[64.0, 120], [64.0, 120], [64.0, 120], [64.5, 120], [64.0, 120], [63.5, 120]],
};

export class SheepRenderer {
  private readonly animations = new Map<SheepState, LoadedAnimation>();
  ...
  draw(ctx: CanvasRenderingContext2D, sheep: Sheep): void {
    ...
    const dt = Math.min(Math.max(0, (now - this.lastTime) / 1000), 0.05);
    this.lastTime = now;
    ...
    while (this.animationTime >= animation.frameDuration) {
      ...
      this.currentFrame = (this.currentFrame + 1) % animation.images.length;
    }
    ...
    const targetCenterX = sheep.x + sheep.width / 2;
    const drawX = Math.round(targetCenterX - sourceCenterX * scale);

    // Alle erzeugten Schaf-Frames haben eine sichtbare Unterkante bei Y=120.
    // Dadurch steht die sichtbare Figur exakt auf der Plattform.
    const targetBottomY = sheep.y + sheep.height;
    ...
    if (sheep.facing === "left") {
      // Beim Spiegeln wird der visuelle Mittelpunkt weiterhin exakt verankert.
  ...
  private loadAnimations(): void {
    for (const [state, definition] of Object.entries(ANIMATIONS) as [
      ...
      for (let i = 0; i < definition.count; i++) {
        ...
        const src =
          `/assets/images/enemy/sheep_white/${state}/${String(i).padStart(2, "0")}.png`;
        ...
        image.onerror = () =>
          console.error(`Schaf-Sprite konnte nicht geladen werden: ${src}`);
